...
export default function ProjectsExplorer({ data }: { data: ProjectsData }) {
  ...
  return (
    <div>
      ...
      {status === "past" ? (
        <div className="mt-6 rounded-2xl border border-dashed border-opti-line bg-opti-navy p-10 text-center">
          <div className="mx-auto flex h-12 w-12 items-center justify-center rounded-xl bg-opti-chip text-xl font-bold text-opti-teal">
            ⌛
          </div>
          <h3 className="mt-3 text-[16px] font-bold text-opti-ink">
            Past project data — placeholder
          </h3>
          <p className="mx-auto mt-1.5 max-w-md text-[13.5px] text-opti-muted">
            Completed and historical projects will appear here once their
            rosters are added. The same team, PM, account-manager and
            delivery-manager filters will apply automatically.
          </p>
        </div>
      ) : (
        <>
